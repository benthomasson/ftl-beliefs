...
from .parser import (
    ...
    parse_nogoods, append_nogood, is_generated,
)
...
# Commands that always write to beliefs.md
_WRITE_COMMANDS = {"add", "update", "add-batch", "add-repo", "hash-sources"}


def _check_read_only(args) -> None:
    """Error if beliefs.md is generated and the command would write to it."""
    if args.command in _WRITE_COMMANDS and is_generated(args.registry):
        print(
            f"Error: {args.registry} was generated by `reasons export-markdown` "
            f"and is read-only. Use `reasons` to modify beliefs, then re-export.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_check_circular(args):
    ...
    if self_deps:
        ...
        if args.flag:
            if is_generated(args.registry):
                print(
                    f"Error: {args.registry} is read-only (generated by reasons). "
                    f"Cannot flag claims.",
                    file=sys.stderr,
                )
                sys.exit(1)
            for cid in self_deps:
